...
function afficher_noms_equipes_inscrites($IdInscriptionTournois)
{
    ...
    $sql = "SELECT * FROM Inscription_Tournoi AS i JOIN Equipe AS e ON e.ID_Equipe = i.FK_ID_Equipe WHERE FK_ID_Tournoi = " . $IdInscriptionTournois;
    $req = $bdd->prepare($sql);
    ...
    foreach ($reponse2 as $data) {
        echo "<option valeur=" . $data['Nom_Equipe'] . " selected>" . $data['Nom_Equipe'] . "</option>";
    }
    echo '</select>';
    return $data;
}
...
function selectionner_noms_equipes_inscrites()
{
    ...
    $sql = "SELECT * FROM Inscription_Tournoi";
    $req = $bdd->prepare($sql);
...
function update_statut_equipes_tournoi($FK_ID_Equipe)
{
    $bdd = connectDB();
    $bdd->query("SET NAMES 'utf8'");

    $reponse = $bdd->query("UPDATE Inscription_Tournoi SET Statut_Inscription_Tournoi  = 'Validé' WHERE ID_Inscription_Tournoi` = '$FK_ID_Equipe'");
    $reponse->setFetchMode(PDO::FETCH_BOTH);
...
function update_suppresion_logique($IdTournoi)
{
    $bdd = connectDB();
    $bdd->query("SET NAMES 'utf8'");
